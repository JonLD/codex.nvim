...
local disable_lsp_for_buffer
local detect_filetype

---Disable treesitter highlight and parsing for a buffer.
---@param buf number
local function disable_treesitter_for_buffer(buf)
  if vim.treesitter and type(vim.treesitter.stop) == "function" then
    pcall(vim.treesitter.stop, buf)
  end

  vim.b[buf].ts_disable = true
  vim.b[buf].ts_highlight = false
end

---Set filetype without triggering FileType autocmds (avoids LSP attach).
---@param buf number
---@param filetype string
local function set_filetype_safely(buf, filetype)
  ...
  vim.api.nvim_buf_call(buf, function()
    vim.cmd("noautocmd setfiletype " .. filetype)
  end)
end

---Create a read-only scratch buffer with file contents to avoid LSP attach.
---@param file_path string
---@return number buf
local function create_readonly_buffer_from_file(file_path)
  local buf = vim.api.nvim_create_buf(false, true)
  if buf == 0 then
    error({
      code = -32000,
      message = "Buffer creation failed",
      data = "Could not create original content buffer",
    })
  end
  ...
  if not ok then
    ...
    error({
      ...
      data = "Could not read original file: " .. tostring(file_path),
    })
  end

  vim.api.nvim_buf_set_name(buf, file_path .. " (Original)")
  ...
  disable_lsp_for_buffer(buf)
  disable_treesitter_for_buffer(buf)
  set_filetype_safely(buf, detect_filetype(file_path, buf))

  return buf
end

---Detach LSP clients and prevent future LSP attachment for a buffer.
---@param buf number
disable_lsp_for_buffer = function(buf)
  if not vim.lsp then
    return
  end
  ...
  vim.api.nvim_create_autocmd("LspAttach", {
    ...
    callback = function(args)
      if args and args.data and args.data.client_id then
        pcall(vim.lsp.buf_detach_client, buf, args.data.client_id)
      end
    end,
    desc = "Detach LSP from Codex diff buffer",
  })
...
detect_filetype = function(path, buf)
...
function M._open_native_diff(old_file_path, new_file_path, new_file_contents, tab_name)
  ...
  vim.cmd("noautocmd edit " .. vim.fn.fnameescape(old_file_path))
  vim.cmd("diffthis")
  vim.cmd("vsplit")
  vim.cmd("noautocmd edit " .. vim.fn.fnameescape(tmp_file))
  ...
  set_filetype_safely(proposed_buf, old_filetype)
...
function M._resolve_diff_as_saved(tab_name, buffer_id)
  ...
  -- Reload the original file buffer after a delay to ensure Codex CLI has written the file
...
function M._create_diff_view_from_window(target_window, old_file_path, new_buffer, tab_name, is_new_file)
  ...
  if is_new_file then
    ...
    original_buffer = create_readonly_buffer_from_file(old_file_path)
  ...
  set_filetype_safely(new_buffer, original_ft)
  vim.cmd("diffthis")
  ...
  vim.b[new_buffer].codex_diff_tab_name = tab_name
  vim.b[new_buffer].codex_diff_new_win = new_win
  vim.b[new_buffer].codex_diff_target_win = target_window
...
function M._cleanup_diff_state(tab_name, reason)
  ...
  -- Clean up the new buffer only (proposed changes)
  ...
  -- Clean up original scratch buffer if we created it
  if diff_data.original_buffer_created_by_plugin and diff_data.original_buffer then
    if vim.api.nvim_buf_is_valid(diff_data.original_buffer) then
...
function M._setup_blocking_diff(params, resolution_callback)
  ...
  local setup_success, setup_error = pcall(function()
    ...
    disable_lsp_for_buffer(new_buffer)
    disable_treesitter_for_buffer(new_buffer)
...
function M.deny_current_diff()
  local current_buffer = vim.api.nvim_get_current_buf()
  local tab_name = vim.b[current_buffer].codex_diff_tab_name
  local new_win = vim.b[current_buffer].codex_diff_new_win
  local target_window = vim.b[current_buffer].codex_diff_target_win
