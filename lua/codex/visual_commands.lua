-- This file provides helpers to extract file lists from visual selections.
...
function M.get_tree_state()
  ...
  if current_ft == "oil" then
    ...
    -- Return the oil.nvim module as the tree state.
    return oil, "oil"
  end
...
function M.capture_visual_selection_data()
  ...
  -- Preserve the line range before leaving visual mode so later sends keep line numbers.
...
function M.get_files_from_visual_selection(visual_data)
  ...
  elseif tree_type == "oil" then
    -- Extract selection paths from oil.nvim display lines.
