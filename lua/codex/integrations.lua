-- This file provides integration handlers for neo-tree and oil.nvim.
--- Tree integration module for Codex.nvim (neo-tree and oil.nvim).
---@module 'codex.integrations'
local M = {}
local logger = require("codex.logger")

---Get selected files when the current buffer is neo-tree or oil.
...
function M._get_oil_selection()
  ...
  if mode == "V" or mode == "v" or mode == "\22" then
    -- Collect oil.nvim entries from the visual line range.
    ...
    -- In normal mode, only handle the entry under the cursor.
