-- This file tracks editor selections and sends them to Codex.
...
function M.disable()
  ...
  M.state.latest_visual_selection = nil
  M.server = nil
...
function M.update_selection()
  ...
  -- Do not update selection state for the Codex terminal buffer.
  if terminal_buffer.is_codex_terminal_buffer(current_buf) then
    -- Clear pending demotion when switching to the Codex terminal.
  ...
  if current_mode == "v" or current_mode == "V" or current_mode == "\022" then
    ...
    if current_selection then
      ...
      M.state.latest_visual_selection = vim.deepcopy(current_selection)
    else
...
---Safely clone tables, including nested tables.
---@param value any
---@return any
...
---Send any pending selection notification and clear stored content.
...
---Gets the latest recorded visual selection.
---@return table|nil The latest visual selection object, or nil if none recorded.
function M.get_latest_visual_selection()
  return M.state.latest_visual_selection
end
...
---@Build selection info with file notation aligned to @mention insertion.
...
---Build content to send to the terminal when Codex is not connected.
---@param selection table
---@return string
...
function M.send_at_mention_for_visual_selection(line1, line2)
  ...
  -- Notify the selection first so Codex can insert text correctly.
  local is_connected = true
  if type(codex_main.is_codex_connected) == "function" then
    is_connected = codex_main.is_codex_connected()
  end

  -- Align file notation so @mentions match selection notifications in Codex.
  ...
  if is_connected then
    ...
    -- Store selection content to send after the connection is established.
    M.state.pending_selection = safe_deepcopy(selection_for_insert)

    -- When Codex is not connected, send the content to the terminal and update input.
    ...
    if allow_fallback then
      ...
      if payload ~= "" then
        -- Sending to the terminal updates the input content.
