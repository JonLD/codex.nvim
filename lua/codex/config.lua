-- This file defines Codex.nvim default settings and validation to safely apply configuration.
---@brief [[
...
M.defaults = {
  ...
  autoinstall = false,
  log_level = "info",
  -- Default to optional so we can connect to CLIs that do not require auth.
  auth_mode = "optional",
  -- Whether to send content to the terminal when Codex is not connected.
...
function M.validate(config)
  ...
  assert(
    (cmd_type == "string" and config.cmd ~= "") or cmd_type == "table",
    "cmd must be a non-empty string or table"
  )
  ...
  if config.model ~= nil then
    assert(type(config.model) == "string", "model must be a string or nil")
  end
