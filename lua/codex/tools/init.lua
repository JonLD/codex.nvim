-- This file implements the MCP tools init and registers handlers.
...
function M.get_tool_list()
  local tool_list = {}
  local call_note =
    " Input must be the arguments object only (do not wrap with name/arguments). Example: {\"filePath\":\"/path/to/file\"}."
  ...
  for name, tool_data in pairs(M.tools) do
    ...
    if tool_data.schema then
      local tool_def = {
        name = name,
        description = (tool_data.schema.description or "") .. call_note,
...
function M.handle_invoke(client, params) -- client needed for blocking tools
  ...
  logger.debug("tools", "handle_invoke params", vim.inspect(params))
  ...
  local function normalize_input(raw_input)
    ...
    if type(normalized) == "table" then
      if type(normalized.name) == "string" and type(normalized.arguments) == "table" then
        tool_name = normalized.name
        normalized = normalized.arguments
      elseif type(normalized.arguments) == "table" then
        normalized = normalized.arguments
      elseif type(normalized.params) == "table" then
  ...
  if type(tool_name) == "table" then
    if type(tool_name.name) == "string" then
      if input == nil and type(tool_name.arguments) == "table" then
        input = tool_name.arguments
      end
      tool_name = tool_name.name
    elseif type(tool_name[1]) == "string" then
      if (input == nil or vim.tbl_isempty(input)) and type(tool_name[2]) == "table" then
        input = tool_name[2]
      end
  ...
  if type(tool_name) ~= "string" and type(params.tool) == "table" then
    if type(params.tool.name) == "string" then
      if input == nil and type(params.tool.arguments) == "table" then
        input = params.tool.arguments
      end
  ...
  if type(tool_name) ~= "string" and type(params[1]) == "string" then
    tool_name = params[1]
    if input == nil and type(params[2]) == "table" then
      input = params[2]
    end
  end
  ...
  if type(tool_name) ~= "string" then
    return {
      error = {
        ...
        data = "Tool name must be a string",
      },
    }
  end
